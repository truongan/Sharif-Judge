...
class Submit extends CI_Controller
{
	...
	public function __construct()
	{
		...
		ob_start();
		ob_end_clean();
		$this->load->model('dsbaitap_model');
	...
	private function _upload()
	{
		...
		$problem = $this->dsbaitap_model->problem_info($this->input->post('problem'));
		// foreach ($filetypes as &$filetype)
		// {
		// 	$filetype = $this->_language_to_type(strtolower(trim($filetype)));
		// }
		if($this->input->post('check_com')=='')
			$check_com = '0';
		else
			$check_com = '1';
		// if ( ! in_array($this->filetype, $filetypes))
		// 	show_error('This file type is not allowed for this problem.');
		$user_dir = rtrim($this->assignment_root, '/').'/assignment_1'.'/p'.$problem['id'].'/'.$this->user->username;
